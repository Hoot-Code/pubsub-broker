...
quickstart شامل یک جادوگر تعاملی تنظیم احراز هویت است که به شما اجازه می‌دهد:
- **خودکار** — یک کلید API امن به طور خودکار تولید کنید (توصیه شده)
- **دستی** — کلید API خود را وارد کنید (حداقل 32 کاراکتر)
- **غیرفعال** — احراز هویت را فقط برای توسعه رد کنید
...
**جریان احراز هویت:**
- کاربران بدون احراز هویت صفحه ورود را در `/dashboard` مشاهده می‌کنند
- کوکی‌های نشست `HttpOnly`، `SameSite=Strict` هستند و پس از 12 ساعت منقضی می‌شوند (از طریق `network.dashboard_session_ttl` قابل پیکربندی)
- خروج از سیستم نشست را در سمت سرور و کوکی را در سمت کلاینت پاک می‌کند
- نشست‌های منقضی شده به طور خودکار به صفحه ورود هدایت می‌شوند
